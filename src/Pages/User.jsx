...
const LoadingStyle={...ContainerStyle, display:"grid",placeContent: "center"}
  ...
  return userMails?(
      <Grid item xs={12} sx={ContainerStyle} >
        ...
        <h1 style={{textAlign:"center"}}> {props.match.params.id} </h1>
         {
           userMails.map((item,index)=>{return <Mail key={index} item={item}  />}) 
         }
        </Container>
      </Grid>

  ):( <Grid item xs={12} sx={LoadingStyle} ><Loading /></Grid>);
};
